...
export const resetOwnerBySession = async (sessionName: string) => {
  ...
  await Users.update({ tipo: 'comum' }, { where: { tipo: 'donosession' } });
};

export const updateOwnerBySession = async (sessionName: string, id_usuario: string) => {
...
export const registerOwnerBySession = async (sessionName: string, id_usuario: string) => {
  await resetOwnerBySession(sessionName);
  await updateOwnerBySession(sessionName, id_usuario);
};

export const getOwnerBySession = async (sessionName: string): Promise<string | null> => {
  getClientDB(sessionName);

  const owner = await Users.findOne({ where: { tipo: 'donosession' } });
  return owner?.id_usuario || null;
};
...
export const getPack = async (id_usuario: string): Promise<string | null> => {
  if (!id_usuario) return null;
  const pack = await Users.findOne({
    where: {
      [Op.or]: [{ id_usuario: id_usuario }, { id_lid: id_usuario }],
    },
  });
...
export const changeWarning = async (id_usuario: string, advertencia: number) => {
  ...
  if (!user) return;
  await user.update({ advertencia: user.advertencia + advertencia });
};

export const getUserWarning = async (id_usuario: string): Promise<number> => {
  if (!id_usuario) return 0;
  const advertencia = await Users.findOne({
    where: {
      [Op.or]: [{ id_usuario: id_usuario }, { id_lid: id_usuario }],
    },
  });
