...
export const checkingMessage = async (
  ...
  if (commandExists.exists) {
    ...
    if (userId) {
      ...
      const permitido = await verificarCooldown(sock, userId, tipo, pushName ?? '', id_chat!);
      if (!permitido) return;
    }
    ...
    if (userId && dataBot.xp?.status && commandName !== 's') {
      await addXpForInteraction(userId, id_chat, sock);
    }
  ...
  if (autostickerpv || autostickergp) {
    if (userId && dataBot.xp?.status) {
      await addXpForInteraction(userId, id_chat, sock, 'sticker_create');
    }
...
async function addXpForInteraction(
  ...
  setImmediate(async () => {
    try {
      ...
      const res: any = await XPService.addEvent(sock.session_name, sender, event as XPEventType);
