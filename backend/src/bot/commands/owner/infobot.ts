...
function getNextXpResetFormatted(): string {
  const now = new Date();
  const next = new Date(now.getFullYear(), now.getMonth() + 1, 1, 0, 0, 0, 0);
  return timestampForData(next.getTime()) ?? '';
}
...
const command: Command = {
  ...
  exec: async (
    ...
    const xpResetDate = getNextXpResetFormatted();
    ...
    resposta += dataBot.xp?.status
      ? createText(textMessage.admin.infobot.msgs.resposta_variavel.xp.on, xpResetDate)
