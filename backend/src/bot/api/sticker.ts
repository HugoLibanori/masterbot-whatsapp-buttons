...
import { Resposta } from '../../interfaces/index.js';
    ...
    (async () => {
      try {
        ...
        if (midiaAnimada && tipo === 'quadrado') {
          ...
          buffer = await editImage(buffer, tipo);
...
async function editImage(buffer: Buffer, tipo: string) {
...
export const stickerFroImage = async (bufferSticker: Buffer): Promise<Resposta> => {
  const resposta: Resposta = {};

  try {
    const bufferImagem = await sharp(bufferSticker).png().toBuffer();
    resposta.resultado = bufferImagem;
    return resposta;
  } catch (err: any) {
    console.error('Erro ao converter sticker com sharp:', err.message);
    return { erro: 'Falha ao converter sticker para imagem (sharp)' };
  }
};

export const stickerFroGif = (bufferSticker: Buffer): Promise<Resposta> => {
  return new Promise((resolve, reject) => {
    const resposta: Resposta = {};
    const entradaWebp = getPathTemp('webp');
    const saidaGif = getPathTemp('mp4');

    try {
      fs.writeFileSync(entradaWebp, bufferSticker);

      exec(`convert ${entradaWebp} ${saidaGif}`, (error, stdout, stderr) => {
        // Remove o arquivo webp sempre
        if (fs.existsSync(entradaWebp)) fs.unlinkSync(entradaWebp);

        if (error) {
          if (fs.existsSync(saidaGif)) fs.unlinkSync(saidaGif);
          resposta.erro = 'Falha ao converter sticker para GIF';
          return reject(resposta);
        }

        try {
          const gifBuffer = fs.readFileSync(saidaGif);
          if (fs.existsSync(saidaGif)) fs.unlinkSync(saidaGif);
          resposta.resultado = gifBuffer;
          resolve(resposta);
        } catch (readErr) {
          resposta.erro = 'Erro ao ler arquivo GIF gerado';
          reject(resposta);
        }
      });
    } catch (err: any) {
      if (fs.existsSync(entradaWebp)) fs.unlinkSync(entradaWebp);
      if (fs.existsSync(saidaGif)) fs.unlinkSync(saidaGif);
      console.error('Erro interno no stickerFroGif:', err);
      reject({ erro: 'Erro interno ao processar o sticker.' });
    }
  });
};
...
export const removeBackground = async (bufferMidia: Buffer, apikey: string): Promise<Buffer> => {
  try {
    const base64img = bufferMidia.toString('base64');
    ...
    const bufferResultado = Buffer.from(result.base64img, 'base64');

    return bufferResultado;
  } catch (errors: any) {
    // A biblioteca oficial retorna um array de erros
    console.error('Erro no Remove.bg:', JSON.stringify(errors));
    throw { erro: 'Não foi possível remover o fundo da imagem.' };
  }
};
