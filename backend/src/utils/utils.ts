...
export async function connectionPairingCode(
  client: types.MyWASocket,
  phone?: string,
): Promise<string | null> {
  let answerNumber = phone;
  if (!answerNumber) {
    answerNumber = await question('Digite o seu telefone (com DDD): ');
  }
  if (!answerNumber) {
    console.log('Nenhum telefone informado para gerar o código de pareamento.');
    return null;
  }
  const normalized = answerNumber.replace(/\W+/g, '');
  if (!normalized) {
...
export {
  getBrasileirao as getBrasileiraoA,
  getChampionshipId,
  formatResumo,
