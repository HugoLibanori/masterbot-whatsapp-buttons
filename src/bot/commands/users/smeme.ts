...
const command: Command = {
  ...
  exec: async (
    ...
    try {
      ...
      const { resultado: resultadoSticker } = await createNameSticker(bufferMeme.resultado!, {
...
async function addTextToImage(
  ...
  if (topText) {
    ...
    if (textWidth > image.width - 20) {
      while (textWidth > image.width - 20 && fontSize > 20) {
        fontSize -= 2;
        ctx.font = `${fontSize}px Impact`;
        textWidth = ctx.measureText(topText).width;
      }
    } else if (textWidth < image.width - 20) {
      if (image.width > 1000) {
        while (textWidth < image.width - 20 && fontSize < 400) {
          fontSize += 1;
          ctx.font = `${fontSize}px Impact`;
          textWidth = ctx.measureText(topText).width;
        }
      } else {
        while (textWidth < image.width - 20 && fontSize < 200) {
          fontSize += 1;
          ctx.font = `${fontSize}px Impact`;
          textWidth = ctx.measureText(topText).width;
        }
